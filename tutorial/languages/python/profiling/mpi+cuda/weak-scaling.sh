...
elapsed_times = []
mpi_prefix_args = '--gpus-per-task=1'
cmd_extra_args = ''
start_ntask_idx = 0
stop_ntask_idx = 8
...
for ntask in ntasks[start_ntask_idx:stop_ntask_idx]:
    # Build the cmd.
    problem_size_arg = [str(ntask*100000)]
    cmd = shlex.split('mpirun') + shlex.split(f'-n {ntask}') + shlex.split(mpi_prefix_args) + shlex.split('python ./test.py') + problem_size_arg + shlex.split(cmd_extra_args)
